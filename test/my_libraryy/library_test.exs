defmodule MyLibraryy.LibraryTest do
  use MyLibraryy.DataCase
  ...
  describe "books" do
    ...
    import MyLibraryy.LibraryFixtures
    ...
    test "create_book/1 with valid data creates a book" do
      valid_attrs = %{title: "some title", author: "some author", isbn: "some isbn", published_at: ~D[2025-07-21]}

      assert {:ok, %Book{} = book} = Library.create_book(valid_attrs)
      assert book.title == "some title"
      assert book.author == "some author"
      assert book.isbn == "some isbn"
      assert book.published_at == ~D[2025-07-21]
    end
    ...
    test "update_book/2 with valid data updates the book" do
      book = book_fixture()
      update_attrs = %{title: "some updated title", author: "some updated author", isbn: "some updated isbn", published_at: ~D[2025-07-22]}

      assert {:ok, %Book{} = book} = Library.update_book(book, update_attrs)
      assert book.title == "some updated title"
      assert book.author == "some updated author"
      assert book.isbn == "some updated isbn"
      assert book.published_at == ~D[2025-07-22]
    end
  ...
  describe "loans" do
    ...
    test "create_loan/1 with valid data creates a loan" do
      valid_attrs = %{borrowed_at: ~D[2025-07-21], due_at: ~D[2025-07-21], returned_at: ~D[2025-07-21]}

      assert {:ok, %Loan{} = loan} = Library.create_loan(valid_attrs)
      assert loan.borrowed_at == ~D[2025-07-21]
      assert loan.due_at == ~D[2025-07-21]
      assert loan.returned_at == ~D[2025-07-21]
    end
    ...
    test "update_loan/2 with valid data updates the loan" do
      loan = loan_fixture()
      update_attrs = %{borrowed_at: ~D[2025-07-22], due_at: ~D[2025-07-22], returned_at: ~D[2025-07-22]}

      assert {:ok, %Loan{} = loan} = Library.update_loan(loan, update_attrs)
      assert loan.borrowed_at == ~D[2025-07-22]
      assert loan.due_at == ~D[2025-07-22]
      assert loan.returned_at == ~D[2025-07-22]
    end
